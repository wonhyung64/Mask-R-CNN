...
def preprocessing(data, batch_size, final_height, final_width, evaluate, augmentation=False):
    ...
    for i in range(batch_size):
        ...
        gt_mask_[i] = tf.cast(gt_mask_[i], tf.float32)
        
        img_[i] = tf.image.convert_image_dtype(img_[i], tf.float32)
        img_[i] = tf.image.resize(img_[i], (final_height, final_width))
        gt_mask_[i] = tf.image.resize(gt_mask_[i], (final_height, final_width), method='nearest')

        img_[i] = tf.reshape(img_[i], shape=(1, final_height, final_width, 3))
        gt_mask_[i] = tf.reshape(gt_mask_[i], shape=(1, final_height, final_width, 1))
        ...
        if np.logical_and(augmentation == True, np.random.uniform(0,1,1) > 0.5) == True:
            ...
            gt_mask_[i] = tf.image.flip_left_right(gt_mask_[i])
    ...
    gt_mask = tf.concat([gt_mask_[i] for i in range(batch_size)], axis=0)
    ...
    return img, gt_boxes, gt_labels, gt_mask
